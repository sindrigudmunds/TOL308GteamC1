...
Pooka.prototype.gridX = Math.floor(this.cx / 32);
Pooka.prototype.gridY = Math.floor(this.cy / 32);
// Smá offset til að hann passi betur inn í göngin.
// Annars stendur hann alltaf smá uppúr.
Pooka.prototype.yOffset = 12;
//Player.prototype.cx = 480;
//Player.prototype.cy = 480;
Pooka.prototype.direction = 'right';
Pooka.prototype.pathToPlayer = [];
Pooka.prototype.isMoving = true;

Pooka.prototype.GetNextDirection = function(){
...
Pooka.prototype.update = function (du) {
  ...
  // Check if Pooka has entered a new cell
...
Pooka.prototype.render = function (ctx) {
  ...
  //g_sprites.pooka.drawAt(ctx,this.cx,this.cy);
};
...
Pooka.prototype.upAnim = function (ctx){
  if(this.animCounter < 8){
    g_sprites.pookaLeft1.drawAt(ctx,this.cx,this.cy);
  } else { // if playerAnimCounter > 8
    g_sprites.pookaLeft2.drawAt(ctx,this.cx,this.cy);
...
Pooka.prototype.downAnim = function (ctx){
  if(this.animCounter < 8){
    g_sprites.pookaLeft1.drawAt(ctx,this.cx,this.cy);
  } else { // if playerAnimCounter > 8
    g_sprites.pookaLeft2.drawAt(ctx,this.cx,this.cy);
